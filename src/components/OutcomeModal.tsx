...
export default function OutcomeModal() {
  ...
  return (
    <div id="outcome_modal_overlay" className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl flex flex-col gap-5 relative overflow-hidden animate-scale-up">
        {/* Glow decoration */}
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-md sm:text-lg font-extrabold text-white leading-tight">
        ...
        <div className="bg-slate-950 p-4 border border-slate-850 rounded-2xl text-slate-200 text-xs sm:text-sm leading-relaxed font-sans shadow-inner">
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(lastChoiceOutcome.statDeltas).map(([statName, value]) => {
                ...
                if (statName === 'happiness') {
                  ...
                  bgColor = isBeneficial ? 'bg-amber-500/10 border-amber-500/20' : 'bg-rose-500/10 border-rose-500/20';
                  ...
                  bgColor = isBeneficial ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20';
                  ...
                  bgColor = isBeneficial ? 'bg-sky-500/10 border-sky-500/20' : 'bg-rose-500/10 border-rose-500/20';
                  ...
                  bgColor = isBeneficial ? 'bg-purple-500/10 border-purple-500/20' : 'bg-rose-500/10 border-rose-500/20';
                  ...
                  bgColor = isBeneficial ? 'bg-violet-500/10 border-violet-500/20' : 'bg-amber-500/10 border-amber-500/20';
                  ...
                  bgColor = isBeneficial ? 'bg-orange-500/10 border-orange-500/20' : 'bg-emerald-500/10 border-emerald-500/20';
                }

                return (
                  <div key={statName} className={`p-2.5 rounded-xl border flex flex-col gap-1.5 ${bgColor}`}>
        ...
        <button
          ...
          className="w-full bg-teal-600 hover:bg-teal-500 text-white font-bold py-3 px-4 rounded-xl text-xs sm:text-sm tracking-wide shadow-lg hover:shadow-teal-600/10 active:scale-95 transition-all duration-200 cursor-pointer outline-none focus:ring-2 focus:ring-teal-500"
        >
